...
namespace game {
    void RenderMap(SDL_Renderer* renderer, Block worldMap[250][250], int mapWidth, int mapHeight, int camOffSetX, int camOffSetY, int camScale) {
        for(int x = 0; x <= mapWidth; x++) {
            for(int y = 0; y <= mapHeight; y++) {
                // Get current block id and define variables
                int objectId = worldMap[x][y].type;
                int colorId = worldMap[x][y].color;
