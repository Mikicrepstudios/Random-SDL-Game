...
namespace gui {
	void GameInfo(core::MF_Window &window, game::Game &game) {
		...
		std::string gameInfoTexts[16] = {
			...
			"Cur preset: " + std::to_string(game.settings.curPreset + 1),
			"Block color: " + std::to_string(game.settings.blockColor),
			"BG color: " + std::to_string(game.settings.bgColor),
			"", // Empty string for skipped line
			"Cam X: " + std::to_string(game.cam.offSetX),
			"Cam Y: " + std::to_string(game.cam.offSetY),
			"Camscale: " + std::to_string(game.cam.scale),
			"", // Empty string for skipped line
			"Player X: " + std::to_string(game.player.x),
			"Player Y: " + std::to_string(game.player.y),
			"Player color: " + std::to_string(game.settings.playerColor)
		};
		// Draw gameInfo hud
		for(int i = 0; i <= 13; i++) {
