...
/**
 * @brief This namespace contains stuff that are required for Mikicrep Framework
 * to work correctly
 */
namespace core {
struct MF_Window {
  ...
  struct Mouse {
    ...
    bool isPressed =
        false; // Only activated on frame when mouse button got clicked

    // These will update based on which mouse button you press
...
// Big stuff
bool InitWindow(core::MF_Window &window, std::string title, int width,
                int height);
